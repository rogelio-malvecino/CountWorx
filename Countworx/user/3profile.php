...
<html lang="en">
  ...
  <style> 
	...
	$(document).ready(function(){
		...
		$("#btnrowaddworkx").click(function(){
			...
			<tr>\
			<td width='20%'><label for='experiencedlevel'>Experience Level</label><input type='text' class='form-control'></td>\
			...
			<td width='20%'><label for='role'>Role</label><input type='text' class='form-control'></td>\
			...
			<td width='25%'><label for='positionlevel'>Position Level</label><input type='text' class='form-control'></td>\
			...
			<td width='10%'><label for='amount'> Amount </label><input type='number'  min='0' value='0' step='0.01' class='form-control'</td>\
	...
	function getTabledata(){
		$('#tbleducationbackground tr:not(:has(th))').each(function(row, tr){
			...
			var Degree = $(tr).find('td:eq(1)').find('input').val();
			var Years = $(tr).find('td:eq(2)').find('input').val();
			var Location = $(tr).find('td:eq(3)').find('input').val();
			var Certificate = $(tr).find('td:eq(4)').find('input').val();
		...
		$('#tblworkExperienced tr:not(:has(th))').each(function(row, tr){
			...
			var Positionlevel = $(tr).find('td:eq(9)').find('input').val();
		...
		$("#btn_Update").click(function(){
			...
			$.ajax({
				...
				data: {fname:fname,lname:lname,gender:gender,civilstatus:civilstatus,child:child,dob:dob,age:age,email:email,contactno:contactno,alternatecontact:alternatecontact,country:country,region:region,city:city,address:address,educationalBackground:JSON.stringify(educationalBackground),workExperienced:JSON.stringify(workExperienced),trainingattended:JSON.stringify(trainingattended)}, 
				success: function(result){
  ...
  <div class="content-wrapper" style="margin-left: 0px;" >
    ...
    <div class="container">      
      <div class="row">
        ...
        <div class="col-md-2">
          <a href="resume-upload.php" class="btn btn-primary">Upload Resume</a>
 ...
 <!-- for educational background -->
		  ...
		  <!-- for educational background -->
			...
			<h2 class="text-center">Work Experienced</h2>
		  <!-- for work experience -->
		  <div class="jcol-md-5 jwell">
            <div class="form-group">
				<table id="tblworkExperienced">
					...
					?>		
					<tr>
					<td width="20%"><label for="experiencedlevel">Experience Level</label><input type="text" class="form-control" value="<?php echo $row['experiencelevel']?>"></td>
						<?php
						echo "<td width='25%'><label for='corecompetencies'>Core Competencies</label>";
					...
					<td width="20%"><label for="role">Role</label><input type="text" class="form-control" value="<?php echo $row['role']?>"></td>
					...
					<td width="25%"><label for="positionlevel">Position Level</label><input type="text" class="form-control" value="<?php echo $row['positionlevel']?>"></td>
					<?php
						echo "<td width='10%'><label for='currency'>Monthly Salary</label>";
						...
						echo "</td>";
					?>
					<td width="10%"><label for="amount"> Amount </label><input type="number" class="form-control" value="<?php echo $row['amount']?>"></td>
					...
					<td width="25%"><label for="contact">Contact</label><input type="text" class="form-control" value="<?php echo $row['contact']?>"></td>
					...
					<?php
						$index =0;
						echo "<td width='20%'><label for='country'>Country</label><input type='text' id='wcountry".$index."' class='form-control'></td>";
						echo "<td width='20%'><label for='region'>Region</label><input type='text' id='wregion".$index."' class='form-control'></td>";
					?>
					<td width="25%"><label for="positionlevel">Position Level</label><input type="text" class="form-control"></td>
					<?php
						echo "<td width='10%'><label for='currency'>Monthly Salary</label>";
						...
						echo "</td>";
					?>
					<td width="10%"><label for="amount"> Amount </label><input type="number"  min="0" value="0" step="0.01" class="form-control" value="<?php echo $row['amount']?>"></td>
					<td width="20%"><label for="otherbenefit">Other Benefit</label><input type="text" class="form-control"></td>
					<td width="20%"><label for="reference">Reference</label><input type="text" class="form-control"></td>
					<td width="25%"><label for="contact">Contact</label><input type="text" class="form-control"></td>
					<td width="35%"><label for="jobdescription">Job Description</label><textarea class="form-control" rows="1" ></textarea></td>
...
<!-- for seminar attended -->
	   ...
	   <div class="text-center">
                <button class="my-button" type="submit" onclick="return false" id="btn_Update">Update Profile</button>
